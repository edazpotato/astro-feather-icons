...
await fs.copyFile(
	ps.resolve(currentDir, "Props.ts"),
...
for (const [name, innerSVG] of Object.entries(source)) {
	...
	const title = name
		.replace(/(?<=^|-)([a-z0-9])/g, (_0, $1) => $1.toUpperCase())
		.replace(/[^A-Za-z0-9]+/g, " ")
		.replace(/Airplay/, "AirPlay")
		.replace(/Codepen/, "CodePen")
...
await fs.writeFile(ps.resolve(distDir, "index.js"), contentOfIndexJS, "utf8");
